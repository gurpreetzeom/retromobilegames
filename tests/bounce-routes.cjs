...
for(let target=0;target<targets.length;target++){
 ...
 for(let step=0;step<240&&!found;step++){
  const next=[];
  for(const p of beam)for(const dir of [-1,0,1])for(const jump of [false,true]){
   ...
   const key=[Math.round(q.x/3),Math.round(q.y/3),Math.round(q.vy/12),Math.round(q.vx/15),q.radius,q.grounded].join(',');if(seen.has(key))continue;seen.add(key);
  ...
  beam=next.sort((a,b)=>a.rank-b.rank).slice(0,180).map(n=>n.q);
 }
